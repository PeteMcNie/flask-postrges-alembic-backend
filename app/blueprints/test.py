...
@blueprint.route("/test-get-all", methods=["GET"])
def test_get_all():
	test_data = m.TestTable.query.all()
	return jsonify([data.__repr__() for data in test_data])


@blueprint.route("/test-get-one/<number>", methods=["GET"])
...
@blueprint.route("/test-add", methods=["POST"])
def add_row():
	if not request.json:
		abort(400)
	row = m.TestTable(
...
@blueprint.route("/test-update/<number>", methods=["GET", 'POST'])
...
@blueprint.route("/test-delete/<number>", methods=["GET", "POST"])
